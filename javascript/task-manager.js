...
listProject.forEach((item) => {
    for (let i = 0; i < item.tasks.length; i++) {
        if (item.tasks[i].assignee.id === userLoging.id) {
            let checkTask = projectRender.find(project => project.projectName == item.projectName);
            if (checkTask) {
...
function renderTasks() {
    ...
    projectRender.forEach((project) => {
        ...
        document.querySelector("#firstProjectPart").innerHTML += `<div id="projectTitle">
...
// document.querySelector("#editStatusIcon").addEventListener("click", function () {
...
function editStatus(id) {
    const taskEditStt = projectRender.find((project) => project.projectTaskList.find((task) => task.taskId === id));
    const indexEdit = taskEditStt.projectTaskList.find((task) => task.taskId === id)
